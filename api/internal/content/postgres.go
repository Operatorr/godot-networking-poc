...
// and draft=true; on conflict only payload + draft + updated_at change, so the
// stored version stays stable across edits (Publish is what bumps the version).
func (s *PostgresStore) Upsert(def Definition) error {
	payload, err := json.Marshal(def.Payload)
	if err != nil {
		return fmt.Errorf("content: marshal %s/%s payload: %w", def.Kind, def.ID, err)
	}
	_, err = s.db.Exec(
		`INSERT INTO content_definitions (kind, id, version, payload, draft, updated_at)
		      VALUES ($1, $2, 1, $3, true, now())
		 ON CONFLICT (kind, id) DO UPDATE
		    SET payload    = EXCLUDED.payload,
		        draft      = EXCLUDED.draft,
		        updated_at = now()`,
		string(def.Kind), def.ID, payload,
	)
	if err != nil {
