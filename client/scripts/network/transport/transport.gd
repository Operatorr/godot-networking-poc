...
## concrete socket type (the live ENetConnection/ENetPacketPeer; WebSocketPeer
## was retired with ADR 0003) lives BELOW this seam, inside a Transport subclass.
## Everything protocol-level — packet (de)serialization (now the Rust
## ProtocolCodec, rust/protocol via client_ext), reconnect/backoff, and ALL stats
## accounting (bytes_sent_by_type / peer_bytes_sent) — stays ABOVE the seam, in
## NetworkManager, addressing peers only by opaque int peer_id.
##
## The seam is wire-neutral: it carries opaque byte buffers, so the framing/
## delivery substrate can change without touching the bytes on the wire.
##
