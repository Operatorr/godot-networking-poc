...
- **Anti-cheat.** All gameplay state is server-authoritative and in-memory; the server validates
  ...
  never gameplay ([ARCHITECTURE.md](../ARCHITECTURE.md) success criteria).
...
- **The POC's actual goal.** Prove a single headless server sustains **500–1000 concurrent
  players** in one Arena ([ARCHITECTURE.md](../ARCHITECTURE.md)). Per-server *density* is the
...
| Single server per Arena | No spatial broad-phase sharing; broadcast is per-player O(players × entities) each Snapshot tick → O(N²) in players. | **Sharding deferred** ([ARCHITECTURE.md](../ARCHITECTURE.md) §Sharding Strategy is Vision, not built). Density is pushed via AoI + delta + the snapshot scheduler instead → [interest-mgmt-aoi.md](../netcode/interest-mgmt-aoi.md). |
...
- **Target drift.** [ARCHITECTURE.md](../ARCHITECTURE.md) lists "≥20 Hz tick" and "2 KB/s"; the
