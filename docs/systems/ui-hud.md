...
| Leaderboard | `hud/leaderboard.gd` | top **5** by PvP kills, **Tab** expands to top 10; each row is `[class icon] Name - Class {level}` with the name/class/level text tinted by the WoW-inspired class color (`PacketTypes.CLASS_COLORS`) and kills right-aligned; the Local player and a recent kill are cued by a row-background tint (so the class color on the text survives). Class/level/icon are joined per entity from `EntityNameCache` — level rides the broadcast `PLAYER_INFO` (protocol v6+); rows are built in code. | top-left |
