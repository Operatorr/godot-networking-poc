...
**Status:** Implemented (verified 2026-06-10 against code) — AoI radius (1000 enter / 1100 exit, sized to cover the client's view on wide/ultrawide displays),
...
> spatial grid, filters by a radius sized to the client's view (1000 enter / 1100 exit), and sorts the
...
|---|---|---|---|
| `aoi_radius` | **1000.0** (`:19`) | **1000.0** | Enter radius — new entity appears within this distance |
| `aoi_exit_radius` | **1100.0** (`:23`) | **1100.0** | Exit radius — visible entity drops only past this |
| `lod_near_radius` | 400.0 (`:26`) | **400.0** | ≤400u → LOD_NEAR (penalty 0) |
| `lod_mid_radius` | 1000.0 (`:27`) | **1000.0** | ≤1000u → LOD_MID (penalty 4); else LOD_FAR (penalty 8) |
...
> **Gameplay-safety note:** the 1000 enter radius is deliberately kept **≥ `MONSTER_DETECTION_RANGE`
...
> **View-coverage note:** the circular AoI must contain the client's *rectangular* view, or
> entities pop in/out at the screen edges. On a 21:9 (3440×1440) display with the `expand` stretch
> (`client/project.godot:29-33`) and the 1.5 camera zoom (`arena_base.gd:83`), the visible world is
> ≈1720×720u — a horizontal half-extent of ~860u (~955u while sprint-zoomed at 1.35), which exceeded
> the old 800u exit radius and caused left/right edge pop-in. The 1000/1100 radii cover that view
> (only sprint-zoom *corners* at ~1035u may still rarely pop).
...
O(players × entities) scan — are **resolved** (#10 radius 1000/1100; #9 shared spatial grid). What
remains is tuning-under-measurement, not missing mechanism:

| Caveat | Evidence | Consequence |
|---|---|---|
| **Not yet measured at target load.** The 1000/1100 radii and the shared grid are correct in shape but unproven at 500–1000 players. | `server_config.gd:19,23`; `spatial_grid.gd` | The scheduler diagnostics (#15) are now wired precisely so this can be observed under load before further tuning. |
...
- ✅ **Tune the AoI radius (#10)** — once narrowed enter 1000→700 / exit 1100→800 for scale, then
  **raised back to enter 1000 / exit 1100** (2026-06-10) so the circular AoI covers the client's
  rectangular view on wide/ultrawide (21:9) displays, where the horizontal half-extent (~860u, ~955u
  sprinting) exceeded the old 800u exit radius and caused left/right edge pop-in; still
  ≥ `MONSTER_DETECTION_RANGE` 650; `regression_assertions.py` defaults updated in lockstep.
...
- **Validated:** the Local player is never culled (`:272`); pinned despawns can't be budget-dropped (`snapshot_scheduler.gd:99`); the 1000 enter radius stays ≥ `MONSTER_DETECTION_RANGE` so an aggro'd monster is never AoI-hidden.
...
- **Tested:** exercised by the Python bot swarm (`load_testing/`, `baseline`/`target`/`stress`) with `regression_assertions.py` asserting the 1000/1100 cull; no dedicated AoI/scheduler unit test today.
