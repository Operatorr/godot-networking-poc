...
- **Can fail:** packet loss past the 256-deep buffer; knockback is not predicted at all — while the `KNOCKED_BACK` flag is set the local player freezes prediction and follows the server position (`prediction.gd::_follow_server_through_knockback`), with a 4 s comms-loss ceiling (`KNOCKBACK_FOLLOW_MAX_SECONDS`) re-armed by every update still flagging knockback (rising edge + each baseline) as the backstop for a lost falling-edge flag; a predicted dash/cast the server refuses or never receives (the dash bit rides one unreliable packet) — heals via cooldown reconciliation within ~1 RTT; replay can only mismatch if the buffered flags/delta differ from what the server applied — the movement *code* cannot diverge (one compiled crate).
